...
const Conversas = () => {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    return otherUser?.nome?.toLowerCase().includes(searchTerm.toLowerCase()) || conv.servico_solicitado.toLowerCase().includes(searchTerm.toLowerCase());
  });
      ...
      <div className={`${isMobile ? 'px-4 py-4' : 'max-w-6xl mx-auto p-6'}`}>
        ...
        {isMobile && currentConversation ? <div className="h-[calc(100vh-140px)]">
          ...
          <Card className="h-full">
            <ChatInterface conversation={currentConversation} messages={messages} imageUploadInfo={imageUploadInfo} onSendMessage={content => sendMessage(currentConversation.id, content)} onUploadImage={file => uploadImage(currentConversation.id, file)} onSetPrice={price => setPrice(currentConversation.id, price)} onRespondToPrice={accept => respondToPrice(currentConversation.id, accept)} onReportUser={(issueType, description) => {
              const reportedUserId = currentConversation.cliente_id === profile.id ? currentConversation.prestador_id : currentConversation.cliente_id;
          ...
          <div className="flex items-center gap-2 mb-8">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black text-gray-900 uppercase tracking-tighter leading-none">
                Suas <span className="text-orange-500">Conversas</span>
              </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1">
              <Card>
                ...
                <CardContent>
                  ...
                  </div> : filteredConversations.length === 0 ? <div className="text-center py-8">
                    <MessageCircle className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                    <h3 className="text-lg font-semibold mb-2">
                      {searchTerm ? 'Nenhuma conversa encontrada' : 'Nenhuma conversa ainda'}
                    </h3>
                    <p className="text-gray-600 text-sm">
                      {searchTerm ? 'Tente buscar por um nome diferente.' : 'Suas conversas com prestadores aparecerão aqui.'}
                    </p>
                  </div> : <ConversationList conversations={filteredConversations} currentUserId={profile.id} onConversationSelect={handleConversationSelect} />}
                </CardContent>
              </Card>
            </div>
            ...
            <div className="lg:col-span-2 hidden lg:block">
              <Card className="h-[600px]">
                {currentConversation ? <ChatInterface conversation={currentConversation} messages={messages} imageUploadInfo={imageUploadInfo} onSendMessage={content => sendMessage(currentConversation.id, content)} onUploadImage={file => uploadImage(currentConversation.id, file)} onSetPrice={price => setPrice(currentConversation.id, price)} onRespondToPrice={accept => respondToPrice(currentConversation.id, accept)} onReportUser={(issueType, description) => {
                  const reportedUserId = currentConversation.cliente_id === profile.id ? currentConversation.prestador_id : currentConversation.cliente_id;
                  reportUser(currentConversation.id, reportedUserId, issueType, description);

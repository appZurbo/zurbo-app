...
const HeroSection = ({
  ...
  return <div className="relative orange-gradient text-white py-24 overflow-hidden">
      <div className="absolute inset-0 bg-black/10"></div>
      
      {/* Floating elements for visual appeal */}
      <div className="absolute top-20 left-10 w-20 h-20 bg-white/10 rounded-full animate-bounce-slow"></div>
      <div className="absolute top-40 right-20 w-12 h-12 bg-white/20 rounded-full animate-bounce-slow" style={{
      animationDelay: '1s'
    }}></div>
      <div className="absolute bottom-20 left-1/4 w-8 h-8 bg-white/15 rounded-full animate-bounce-slow" style={{
      animationDelay: '2s'
    }}></div>
        ...
        animationDelay: '0.3s'
          ...
          <div className="flex flex-col sm:flex-row gap-4 max-w-3xl mx-auto mb-16">
            <div className="relative flex-1">
              ...
              <Input placeholder="Que serviço você precisa?" className="pl-12 h-14 text-gray-900 text-lg border-0 shadow-lg" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyPress={handleKeyPress} />
            </div>
            <div className="relative flex-1">
              <MapPin className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
              <Input placeholder="Sua localização" className="pl-12 h-14 text-gray-900 text-lg border-0 shadow-lg" />
            </div>
            <Button size="lg" className="bg-white text-orange-500 hover:bg-orange-50 h-14 px-8 text-lg font-semibold shadow-lg hover-lift" onClick={handleSearch}>
              Buscar Agora
            </Button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-fade-in-up" style={{
        animationDelay: '0.6s'
      }}>
          ...
          animationDelay: '0.2s'
            ...
            <div className="aspect-square mb-4 rounded-xl overflow-hidden">
              <img src="https://images.unsplash.com/photo-1518770660439-4636190af475?w=400&h=400&fit=crop&crop=center" alt="Serviço de Eletricista" className="w-full h-full object-cover" />
            </div>

...
import { User, Settings, LogOut, Bell, Home, Users, Crown, MessageCircle, Calendar, BarChart3, Shield, ShoppingBag, Wrench } from 'lucide-react';
  ...
  return <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Desktop Navigation */}
          {!isMobile && <nav className="hidden md:flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            {isAuthenticated ? <>
                ...
                {!isPremium && <Button variant="ghost" size="sm" onClick={() => navigate('/planos')} className="text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50">
                    {isMobile ? <Crown className="h-4 w-4" /> : <>
                ...
                {!isMobile && <Button variant="ghost" size="sm" onClick={() => navigate('/conversas')} className="relative">
                  ...
                  <DropdownMenuContent className="w-56" align="end" forceMount>
                    <div className="flex items-center justify-start gap-2 p-2">
                      <div className="flex flex-col space-y-1 leading-none">
                        ...
                        {isAdmin && <Badge className="w-fit bg-red-100 text-red-800 border-red-200">
                            <Shield className="h-3 w-3 mr-1" />
                            Admin
                          </Badge>}
                        {isPremium && <Badge className="w-fit bg-yellow-100 text-yellow-800 border-yellow-200">
                    ...
                    {/* Client Menu Items */}
                    {!isPrestador && !isAdmin && <DropdownMenuItem onClick={() => navigate('/configuracoes')}>
                ...
                <Button variant="ghost" size="sm" onClick={() => navigate('/planos')} className="text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50">
                  {isMobile ? <Crown className="h-4 w-4" /> : <>
                      <Crown className="h-4 w-4 mr-2" />
                      <span className="font-medium">Premium</span>
                    </>}

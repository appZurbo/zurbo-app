-- Seed with conditional 'active' to avoid violating one-active-per-type constraint
with new_docs as (
  select * from (values
    ('cliente_termos'::text, '2025-08-11'::text,
...
$$::text,
  ...
  ) as t(doc_type, version, content, summary)
)
...
select 
  ...
  t.content,
  t.summary,
  case when exists (select 1 from public.legal_documents d where d.doc_type = t.doc_type and d.active = true) then false else true end as active
from new_docs t
where not exists (
  select 1 from public.legal_documents d2 where d2.doc_type = t.doc_type and d2.version = t.version
);
